...
namespace CoverShooter
{
    ...
    public class CoverSearch
    {
        ...
        private Cover[] _covers = new Cover[32];
        private int _coverCount;

        private Cover[] _coverArea = new Cover[32];
        private int _coverAreaCount;
        private Vector3 _coverAreaPosition;
        private float _coverAreaRadius;
        private bool _hasCoverArea;
        ...
        public void Update(CoverState current, Vector3 position, Vector3 head, float takeRadius, float crouchRadius, float climbRadius, float capsuleRadius, CoverSettings settings)
        {
            ...
            if (!_hasCoverArea ||
                Vector3.Distance(_coverAreaPosition, position) > _coverAreaRadius - searchRadius)
            {
                _hasCoverArea = true;
                _coverAreaPosition = position;
                _coverAreaRadius = searchRadius * 2;

                var colliderCount = Physics.OverlapSphereNonAlloc(_coverAreaPosition, _coverAreaRadius, Util.Colliders, Layers.Cover);
                _coverAreaCount = 0;
                ...
                for (int i = 0; i < colliderCount; i++)
                {
                    ...
                    if (cover != null)
                    {
                        _coverArea[_coverAreaCount++] = cover;

                        if (_coverAreaCount == _coverArea.Length)
                            break;
                    }
                }
            }

            _coverCount = _coverAreaCount;

            for (int i = 0; i < _coverAreaCount; i++)
                _covers[i] = _coverArea[i];
        }
        ...
        private bool doesCoverFit(Cover cover, bool checkDistance)
        {
            ...
            if (checkDistance)
            {
                var isOld = isInFront && distance <= _settings.LeaveDistance + 0.05f && cover == _current.Main;
                if (isOld) return true;

                var isNew = isInFront && distance <= _settings.EnterDistance + 0.05f && cover != _current.Main;
