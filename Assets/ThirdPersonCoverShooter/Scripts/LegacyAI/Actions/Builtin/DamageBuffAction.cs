...
namespace CoverShooter
{
    ...
    public class DamageBuffAction : AnimatedActorAction
    {
        ...
        /// </summary>
        public override bool IsNeededFor(BaseActor target)
        {
            var buff = target.GetComponent<DamageBuff>();
            if (buff != null && buff.enabled) return false;

            var targetAsActor = target as Actor;
            if (targetAsActor == null) return false;

            return targetAsActor.Threat != null && targetAsActor.Threat.IsAlive && (!AutoHelpOnlyCovered || target.Cover != null);
        }
