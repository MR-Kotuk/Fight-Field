...
namespace CoverShooter
{
    ...
    public struct CharacterIK
    {
        ...
        /// Moves bones.
        /// </summary>
        public void Update(IKConfig config, out Vector3 usedAimTarget)
        {
            ...
            var smoothAimTarget = origin + (_relativeAim * bodyAimVector).normalized * _relativeAimDistance;
            ...
            if (gun != null)
            {
                ...
                recoilShift -= distance * gun.transform.forward * _motor.Bloom / 20f;
                recoilShift -= distance * gun.transform.right * _motor.Bloom / 20f;
            }
            ...
            if (config.Sight != null && _headAimIntensity > 0.01f)
            {
                ...
                usedAimTarget = smoothAimTarget;
            }
            else
                usedAimTarget = currentBodyTarget;
        }
