...
namespace CoverShooter
{
    ...
    public class CharacterAlerts : MonoBehaviour, ICharacterPhysicsListener, ICharacterHealthListener
    {
        ...
        public void OnLand()
        {
            Alerts.Broadcast(transform.position, Land, AlertType.Locomotion, _actor, true);
        }
        ...
        {
            Alerts.Broadcast(transform.position, Hurt, AlertType.Hit, _actor, true);
        }
        ...
        public void OnFootstep(Vector3 position)
        {
            if (_motor == null || (!_motor.IsCrouching && !_motor.IsInCover))
                Alerts.Broadcast(transform.position, Step, AlertType.Locomotion, _actor, true);
        }
        ...
        public void OnDead()
        {
            Alerts.Broadcast(transform.position, Death, AlertType.Death, _actor, true);
        }
        ...
        {
            Alerts.Broadcast(transform.position, Resurrect, AlertType.Chat, _actor, true);
        }
        ...
        public void OnJump()
        {
            Alerts.Broadcast(transform.position, Jump, AlertType.Locomotion, _actor, true);
        }
    }
}
